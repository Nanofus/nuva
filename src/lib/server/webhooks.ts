...
export const latestPostHook = async () => {
  ...
  const announcedPosts = latestPosts.filter((post) => post.date.getTime() > latestPost.date.getTime());
  ...
  if (announcedPosts.length > 0) await setLatestPostSlug(announcedPosts[announcedPosts.length - 1].slug);
};

export const latestCommentHook = async () => {
  let latestCommentId = await getLatestCommentId();
  if (!latestCommentId) return;
  const latestComments = (await getLatestComments()).sort((a, b) => a.date.getTime() - b.date.getTime());
